...
def generate_cv():
    ...
    Use DeepSeek to merge answers into selected CV template.
    """
    ...
    payload = {
        "model": "deepseek-chat",
        "messages": [
            {"role": "system", "content": "You are a professional CV formatter."},
    ...
    try:
        ...
        deepseek_text = result_json['choices'][0]['message']['content']
        html_content = clean_html_response(deepseek_text)
...
def generate_cover_letter():
    ...
    Use DeepSeek to merge info into the Cover Letter template.
    """
    data = request.get_json()
...
{job_description}
    ...
    try:
        ...
        print(f"Error calling DeepSeek: {str(e)}")
        return jsonify({"error": f"Failed to generate ATS score: {str(e)}"}), 500
...
{job_description}
    ...
    try:
        ...
        print(f"Error calling DeepSeek: {str(e)}")
        return jsonify({"error": f"Failed to generate resume from job: {str(e)}"}), 500
